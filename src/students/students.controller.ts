...
import type { Student } from "./student.model.ts";
...
export const deleteStudent = async (c: Context) => {
  try {
    ...
    if (result.affectedRows === 0) return c.json({ message: "Student not found" }, 404);

    return c.json({ message: `Deleted ${result.affectedRows} student(s)` });
  } catch (error) {
    console.error(error);
